...
import {
  createActionChangedMessage,
  createActionDeleteMessage,
...
export const Message = ({ message }) => {
  ...
  const handlerDeleteMessage = (id) => {
    dispatch(createActionDeleteMessage(id));
  };
  const valueInInput = useSelector((state) => state.input.value);

  const handlerChangeMessage = (message) => {
    dispatch(createActionChangeValueInInput(message.text));
    dispatch(createActionChangedMessage(message.id));
  };
  return (
    <div className={message.author === 'Я' ? 'message message-me' : 'message'}>
      {message.author === 'Я' ? (
        <div className='message-options'>
          <span
            className='message-change'
            onClick={() => handlerChangeMessage(message)}
          >
            Изменить
          </span>
          <div
            className='message-delete'
            onClick={() => handlerDeleteMessage(message.id)}
          >
            <svg
